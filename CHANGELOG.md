...
### Added
- **TraXile-parity mechanic detection (Phase 6.10)** — closes the detection gap with TraXile, all still from `Client.txt`:
  - **Whole-line substring matcher** — the one mechanism we lacked vs TraXile (it does `line.Contains(key)`). New `LogEvent::SystemLine` surfaces `] : <msg>` system lines and `] [Tag] <msg>` league-tag lines (tag must contain a lowercase letter → excludes ALLCAPS engine tags), and a `by_line` substring table (`encounters::match_line`) is applied to both system lines and NPC dialogue. Unlocks signals the `] Name: text` parser dropped — all verified in a real log: **Mirage** (`[Faridun] Blocking terrain outside mirage area`, the 3.28 league), Nameless Seer, Reflecting Mist, `Be twice blessed.`, and the Simulacrum full-clear (`Strange Voice: So be it. Keep your precious sanity…`, a substring of a longer line that exact matching missed).
  - **Outcome-detail tables** — ported TraXile's exact strings into `by_quote` (`kind:"outcome"`, recorded per occurrence): Ultimatum (Trialmaster) victory/loss/took-reward, Ancestor (Navali) match + tournament win/loss, Sanctum (Lycia) boss kills. Exact-match is used so `"The tournament is over."` can't collide with the longer win line. Several lines are unverified vs the live client (flagged in `encounters.json`).
  - **Area completeness** — `AREA_MECHANICS` gains the 11 Reliquary key areas.
  - **Timed sub-activities** — sub-areas entered inside a map (Vaal side area, Sanctum floor, lab trial, Abyssal Depths, Legion Domain, …) are now tracked with their own start/end/duration while the parent run continues (TraXile's nested-activity model). New `map_subactivities` table (DB migration **v5**) + `MapRun.subactivities`. Returning to the parent map (which logs no `Generating` line) is distinguished from a genuine new sub-area via the seed-based run identity.
  - Honest boundary unchanged: in-map Breach/Legion-monolith/Ritual/Metamorph/Abyss-crack and per-altar Eldritch choices still emit nothing — TraXile misses them too. UI surfacing of the sub-activity/outcome detail is deferred.
